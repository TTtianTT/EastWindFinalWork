...
public class FindPath2 : MonoBehaviour
{
    ...
    {
        Grid.APoint startNode = grid.getItem(s);
        Grid.APoint endNode = grid.getItem(e);

        List<Grid.APoint> openSet = new List<Grid.APoint>();
        HashSet<Grid.APoint> closeSet = new HashSet<Grid.APoint>();
        openSet.Add(startNode);

        while (openSet.Count > 0)
        {
            Grid.APoint curNode = openSet[0];
    ...
    // 生成路径
    void generatePath(Grid.APoint startNode, Grid.APoint endNode)
    {
        List<Grid.APoint> path = new List<Grid.APoint>();
        if (endNode != null)
        {
            Grid.APoint temp = endNode;
